...
describe("PageBuilder", () => {
  ...
  it("renders a realistic mixed block fixture with retained CMS blocks", async () => {
    ...
    expect(
      container.querySelector('[data-testid="mock-block-preview-cards-block"]'),
    ).not.toBeNull();
  });
  ...
  it("opens the mobile structure workspace without stacking it into the canvas", async () => {
    root = createRoot(container);
    ...
    const structureButton = container.querySelector<HTMLButtonElement>(
      '[data-testid="button-mobile-builder-structure"]',
    );
    expect(structureButton).not.toBeNull();
    ...
    expect(
      document.body.querySelector('[data-testid="mobile-builder-structure-sheet"]'),
    ).not.toBeNull();
    expect(document.body.textContent).toContain("Page structure and content");
  });
});
