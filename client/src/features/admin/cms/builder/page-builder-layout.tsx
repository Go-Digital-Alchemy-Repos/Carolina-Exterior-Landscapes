...
export function MobileBuilderLayout({
  ...
  return (
    <div className="xl:hidden">
      <div className="min-h-[58vh] overflow-hidden rounded-2xl border border-border/70 bg-background shadow-sm">
        <VisualCanvas {...canvasProps} />
      </div>

      <Sheet open={structurePanelOpen} onOpenChange={onSetStructurePanelOpen}>
        <SheetContent
          side="left"
          size="full"
          className="h-[100dvh] w-full gap-0 p-0 sm:max-w-md"
          data-testid="mobile-builder-structure-sheet"
        >
          <SheetHeader className="border-b p-4 pr-14">
            <SheetTitle>Page structure and content</SheetTitle>
            <SheetDescription>Browse existing sections or add content to this page.</SheetDescription>
          </SheetHeader>
          <SheetBody className="min-h-0 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
            {leftRailPanel}
          </SheetBody>
        </SheetContent>
      </Sheet>

      <Sheet open={advancedInspectorOpen} onOpenChange={onSetAdvancedInspectorOpen}>
        <SheetContent
          side="right"
          size="full"
          className="h-[100dvh] w-full gap-0 p-0 sm:max-w-xl"
          data-testid="mobile-builder-inspector-sheet"
        >
          <SheetHeader className="border-b p-4 pr-14">
            <SheetTitle>Edit selected section</SheetTitle>
            <SheetDescription>Update the content and settings for the selected page section.</SheetDescription>
          </SheetHeader>
          <SheetBody className="min-h-0 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
            {inspectorPanel}
          </SheetBody>
