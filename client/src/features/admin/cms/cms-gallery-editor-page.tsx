...
type EditableItem = Omit<CmsGalleryItem, "id" | "galleryId" | "createdAt" | "updatedAt"> & {
  id?: string;
};
...
export default function CmsGalleryEditorPage() {
  ...
  useEffect(() => {
    if (loadedGallery)
      setDraft({
        ...loadedGallery,
        settings: { ...DEFAULT_GALLERY_SETTINGS, ...loadedGallery.settings },
      });
  }, [loadedGallery]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const response = await apiRequest(
        isNew ? "POST" : "PUT",
        isNew ? "/api/admin/cms/galleries" : `/api/admin/cms/galleries/${draft.id}`,
        payload,
      );
  ...
  function setSetting<K extends keyof GallerySettings>(key: K, value: GallerySettings[K]) {
    setDraft((current) => ({
      ...current,
      settings: { ...DEFAULT_GALLERY_SETTINGS, ...current.settings, [key]: value },
    }));
  }
  ...
  function addAssets(assets: CmsMediaLibraryAsset[]) {
    ...
    setDraft((current) => ({
      ...current,
      items: [...current.items, ...(newItems as CmsGalleryItem[])],
    }));
  }
      ...
      <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <Button variant="ghost" size="icon" onClick={() => navigate("/admin/cms/galleries")}>
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-semibold">
                  {isNew ? "New Gallery" : draft.title || "Gallery"}
                </h1>
                <Badge
                  variant={draft.status === "published" ? "default" : "outline"}
                  className={draft.status === "published" ? "bg-green-600" : ""}
                >
              ...
              <CardContent className="space-y-4">
                {draft.items.length === 0 ? (
                  ...
                  draft.items.map((item, index) => (
                    <div
                      key={item.id ?? index}
                      className="grid gap-4 rounded-lg border p-4 md:grid-cols-[180px_1fr]"
                    >
                      <CmsImageUpload
                        value={item.imageUrl}
                        onChange={(url) => updateItem(index, { imageUrl: url })}
                        data-testid={`gallery-item-upload-${index}`}
                      />
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        <Input
                          value={item.ctaText ?? ""}
                          onChange={(event) => updateItem(index, { ctaText: event.target.value })}
          ...
          <DialogContent className="max-w-5xl">
            ...
            <article className="max-h-[70vh] overflow-y-auto rounded-lg border bg-background p-6">
              ...
              {previewGallery.description ? (
                <p className="mt-2 text-muted-foreground">{previewGallery.description}</p>
              ) : null}
              <div className="mt-6">
                <GalleryRenderer gallery={previewGallery} preview />
              </div>
      ...
      <CardContent className="space-y-4">
        {["grid", "masonry", "carousel"].includes(layout) ? (
          <div className="grid grid-cols-3 gap-2">
            <NumberField
              label="Desktop columns / shown"
              value={settings.columnsDesktop}
              min={1}
              max={6}
              onChange={(value) => onChange("columnsDesktop", value)}
            />
            <NumberField
              label="Tablet"
              ...
              min={1}
              max={4}
              onChange={(value) => onChange("columnsTablet", value)}
            />
            <NumberField
              label="Mobile"
              ...
              min={1}
              max={2}
              onChange={(value) => onChange("columnsMobile", value)}
            />
          </div>
        ) : null}
        <SelectField
          ...
          onChange={(value) => onChange("spacing", value as GallerySettings["spacing"])}
          options={[
            ["none", "None"],
            ["sm", "Small"],
            ["md", "Medium"],
        ...
        <SelectField
          ...
          onChange={(value) => onChange("imageRatio", value as GallerySettings["imageRatio"])}
          options={[
            ["auto", "Natural"],
            ["1/1", "Square"],
            ["4/3", "4:3"],
        ...
        <SelectField
          ...
          onChange={(value) => onChange("cropMode", value as GallerySettings["cropMode"])}
          options={[
            ["cover", "Crop to fill"],
        ...
        <SelectField
          ...
          onChange={(value) => onChange("borderRadius", value as GallerySettings["borderRadius"])}
          options={[
            ["none", "None"],
            ["sm", "Small"],
            ["md", "Medium"],
        ...
        <SelectField
          ...
          onChange={(value) => onChange("hoverEffect", value as GallerySettings["hoverEffect"])}
          options={[
            ["none", "None"],
            ["zoom", "Zoom"],
            ["fade", "Fade"],
        ...
        <NumberField
          ...
          min={0}
          max={200}
          onChange={(value) => onChange("maxImages", value)}
        />
        <SelectField
          label="Caption position"
          value={settings.captionPosition}
          onChange={(value) =>
            onChange("captionPosition", value as GallerySettings["captionPosition"])
          }
          options={[
            ["below", "Below image"],
            ["overlay", "Overlay"],
        ...
        {slideLike ? (
          <SelectField
            ...
            onChange={(value) =>
              onChange("transitionEffect", value as GallerySettings["transitionEffect"])
            }
            options={[
        ...
        {slideLike || settings.lightbox ? (
          <div className="grid grid-cols-2 gap-2">
            <ColorField
              ...
              onChange={(value) => onChange("arrowIconColor", value)}
            />
            <ColorField
              label="Arrow background"
              value={settings.arrowBackgroundColor}
              onChange={(value) => onChange("arrowBackgroundColor", value)}
            />
          </div>
        ) : null}
        <ToggleField
        ...
        <ToggleField
          ...
          onChange={(value) => onChange("showCaptions", value)}
        />
        <ToggleField
          label="Lightbox"
          checked={settings.lightbox}
          onChange={(value) => onChange("lightbox", value)}
        />
      </CardContent>
    </Card>
  );
