...
export default function CmsMediaPage() {
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onSuccess: ({ deletedIds }) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/cms/media"] });
      toast({
        title: `${deletedIds.length} media item${deletedIds.length !== 1 ? "s" : ""} deleted`,
      });
  ...
  const allVisibleSelected =
    visibleAssets.length > 0 && selectedVisibleCount === visibleAssets.length;
              ...
              <div className="rounded-xl border p-4 space-y-3 bg-muted/10">
                ...
                {selectedAsset.usageRefs.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    No CMS page or global SEO references were found for this media item.
                  </p>
                )}
              </div>
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove {selectedIds.size} selected file
              {selectedIds.size !== 1 ? "s" : ""} from your media library and storage. Any pages or
              blocks that reference these URLs will show broken file links or missing images.
              {selectedAssets.some((asset) => asset.usageCount > 0)
                ? " Some selected files are currently referenced by CMS content."
                : ""}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
