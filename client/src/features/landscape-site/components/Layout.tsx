...
import {
  ...
  COMMERCIAL_SERVICES,
} from "@/features/landscape-site/content/site";
...
const FALLBACK_RESIDENTIAL_ITEMS = [
  ...RESIDENTIAL_SERVICES.map((service) =>
    menuItem(service.slug, service.name, `/${service.slug}`),
  ),
...
const FALLBACK_FOOTER_COMMERCIAL_ITEMS = FALLBACK_COMMERCIAL_ITEMS.filter(
  (item) => item.id !== "commercial-portfolio",
);
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const mobileNavigation = menuItems(publicMenus?.mobile_navigation, mainNavigation);
  const footerResidential = menuItems(publicMenus?.footer_platform, FALLBACK_RESIDENTIAL_ITEMS);
  const footerCommercial = menuItems(
    publicMenus?.footer_secondary,
    FALLBACK_FOOTER_COMMERCIAL_ITEMS,
  );
  ...
  return (
    <div className="landscape-site min-h-screen flex flex-col font-sans selection:bg-primary/20 selection:text-primary">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-2 text-primary-foreground/90">
              <Leaf className="h-4 w-4" /> {BRAND.founded}
            </span>
          </div>
        </div>
      </div>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
          <Link href="/" className="flex items-center group z-50">
            <img
              ...
              fetchPriority="high"
              decoding="async"
              className="h-[3.75rem] md:h-12 w-auto group-hover:opacity-90 transition-opacity"
            />
          </Link>

          {/* Desktop Nav */}
          <nav
            className="hidden lg:flex items-center gap-8 font-bold text-sm text-foreground/80"
            ...
            {mainNavigation.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    id={menuId}
                    className={`absolute top-full left-0 w-72 bg-background border border-border/60 shadow-xl rounded-xl transition-all duration-200 overflow-hidden ${desktopMenuOpen === item.id ? "visible translate-y-0 opacity-100" : "invisible translate-y-2 opacity-0"}`}
                  >
                    <div className="p-3 flex flex-col gap-1">
                      {item.children.map((child) => (
                        <div
                          key={child.id}
                          className="group/link flex items-center rounded-lg hover:bg-muted/50"
                        >
                          <MenuLink
                            item={child}
                            className={`flex-1 p-3 text-sm transition-colors ${child.id === "commercial" ? "font-extrabold" : ""}`}
                          />
        ...
        {mobileMenuOpen ? (
          <div
            ...
            className="fixed inset-0 bg-background z-40 pt-28 px-6 overflow-y-auto"
          >
            <div className="flex flex-col gap-6 font-extrabold text-2xl pb-20">
              ...
              <div className="mt-8 flex flex-col gap-4">
                <a
                  href={`tel:${BRAND.phoneTel}`}
                  className="flex items-center justify-center gap-3 font-extrabold text-foreground bg-muted p-5 rounded-2xl"
                >
                ...
                <Button
                  asChild
                  size="lg"
                  className="w-full text-lg h-16 rounded-2xl shadow-xl shadow-primary/20"
                >
                  <Link href="/get-a-quote">GET A QUOTE</Link>
                </Button>
              </div>
      ...
      <footer className="bg-foreground text-background pt-24 pb-12 relative overflow-hidden">
        ...
        <BotanicalAccent
          variant="fern"
          className="hidden lg:block absolute left-2 bottom-8 h-64 w-auto text-primary/10"
        />
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-16">
            <div className="lg:col-span-4 space-y-8">
              ...
              <p className="text-background/70 text-base leading-relaxed font-medium max-w-sm">
                ...
                We are proud to serve {BRAND.county} and the {BRAND.region} with premium landscaping
              ...
              <div className="space-y-4 font-bold text-sm">
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-background group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                ...
                <div className="flex items-center gap-4 group">
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-background">
            ...
            <div className="lg:col-span-4 lg:col-start-9">
              ...
              <div className="mt-10">
                <Button
                  asChild
                  variant="outline"
                  className="w-full bg-transparent border-white/20 text-white hover:bg-white hover:text-foreground h-12 rounded-full font-bold tracking-wide"
                >
          ...
          <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-6 text-sm font-medium text-background/50">
            ...
            <div className="flex flex-wrap justify-center gap-6">
              <span className="flex items-center gap-2">
                <Leaf className="h-3 w-3" /> {BRAND.founded}
              </span>
