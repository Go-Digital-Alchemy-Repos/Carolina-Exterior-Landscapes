    ...
    <div className={cn("w-full space-y-16", className)}>
      {groups.map((group, idx) => {
        ...
        if (group.type === 'prose') {
          ...
          flushList();
          return (
            <div key={idx} className="prose prose-stone lg:prose-lg max-w-4xl mx-auto prose-headings:font-extrabold prose-h2:text-3xl prose-h2:tracking-tight prose-h3:text-2xl prose-h3:mt-10 prose-h3:mb-4 prose-p:font-medium prose-p:text-muted-foreground prose-p:leading-relaxed prose-li:font-medium prose-li:text-muted-foreground prose-strong:text-foreground">
              {elements}
            </div>
          );
        }
