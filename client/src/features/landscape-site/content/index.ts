...
import type { BlogFaq } from "@shared/blog-faq";
...
export function getBlogImage(filename: string | undefined): string | undefined {
  ...
  const entry = Object.entries(blogImages).find(([path]) => path.endsWith(`/${webpFilename}`));
  return entry?.[1];
}

const pages = pagesData as Record<string, PageContent>;
const locations = locationsData as LocationContent[];
const blog = (blogData as BlogPost[]).slice().sort((a, b) => (a.date < b.date ? 1 : -1));
