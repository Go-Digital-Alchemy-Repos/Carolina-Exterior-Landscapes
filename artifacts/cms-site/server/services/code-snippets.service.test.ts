...
<html>
  ...
  it("moves site verification meta tags from body-start snippets into the head", async () => {
    const { injectSiteCodeSnippets } = await import("./code-snippets.service");
    const html = "<html><head><!--APP_DYNAMIC_HEAD--></head><body><div id=\"root\"></div></body></html>";
    const verification = '<meta name="google-site-verification" content="abc" />';

    const result = injectSiteCodeSnippets(html, {
      head: "",
      header: `${verification}<noscript id="tag-manager"></noscript>`,
      footer: "",
    });

    expect(result.indexOf(verification)).toBeLessThan(result.indexOf("</head>"));
    expect(result.slice(result.indexOf("<body>"))).not.toContain(verification);
    expect(result).toContain('<noscript id="tag-manager"></noscript>');
  });
});
