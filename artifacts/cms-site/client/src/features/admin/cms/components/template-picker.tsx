...
export function TemplatePicker({ open, onClose, onSelect, onOpenWizard }: TemplatePickerProps) {
  ...
  const handleConfirm = () => {
    ...
    if (!tmpl) return;
    onSelect({ blocks: tmpl.blocks }, tmpl.name);
  };
      ...
      <DialogContent className="max-w-2xl" data-testid="dialog-template-picker">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 py-2">
          {PAGE_TEMPLATES.map((tmpl) => {
            const Icon = ICON_MAP[tmpl.id === "landing" ? "Sparkles" : "FileText"] ?? FileText;
            const isSelected = selected === tmpl.id;
            const blockCount = tmpl.blocks.length;
            return (
              <button
                key={tmpl.id}
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mt-auto">
                  <Badge variant="secondary" className={`text-[10px] px-1.5 py-0 ${CATEGORY_COLORS.starter}`}>
                    starter
                  </Badge>
                  {blockCount > 0 && (
