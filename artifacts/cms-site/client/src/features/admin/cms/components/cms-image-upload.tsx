...
const IMAGE_ACCEPTED_TYPES = ["image/png", "image/jpeg", "image/webp", "image/gif", "image/svg+xml", "image/x-icon", "image/vnd.microsoft.icon"];
const IMAGE_ACCEPTED_EXTENSIONS = [".png", ".jpg", ".jpeg", ".webp", ".gif", ".svg", ".ico"];
...
function inferAssetKindFromValue(value: string): "image" | "document" {
  return /\.(png|jpe?g|webp|gif|svg|ico)(?:\?.*)?$/i.test(value) ? "image" : "document";
}
...
export function CmsImageUpload({
      ...
      if (!isAcceptedFile(file, acceptedMode)) {
        throw new Error(
          acceptedMode === "all"
            ...
            : "Only PNG, JPEG, WebP, GIF, SVG, and ICO files are accepted"
        );
      }
  ...
  const acceptAttr =
    acceptedMode === "all"
      ...
      : "image/png,image/jpeg,image/webp,image/gif,image/svg+xml,image/x-icon,image/vnd.microsoft.icon,.svg,.ico";
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {acceptedMode === "all"
                    ...
                    : "PNG, JPG, WebP, GIF, SVG, ICO · Max 10 MB"}
                </p>
              </div>
              <Button
                type="button"
