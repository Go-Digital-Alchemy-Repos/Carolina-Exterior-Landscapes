...
describe("CmsPageEditorPage", () => {
  ...
  it("lets editors update the page title from the builder tab", async () => {
    editorLockState.isReadOnly = false;
    root = createRoot(container);
    ...
    expect(titleInput).not.toBeNull();
    expect(titleInput?.value).toBe("Join the Network");

    await act(async () => {
      const setValue = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set;
      setValue?.call(titleInput, "Updated Builder Title");
      titleInput!.dispatchEvent(new Event("input", { bubbles: true }));
      titleInput!.dispatchEvent(new Event("change", { bubbles: true }));
    });
  ...
  it("saves page edits when the SEO description is longer than 160 characters", async () => {
    editorLockState.isReadOnly = false;
    mockPage.seoDescription = "x".repeat(277);
    mockPage.canonicalUrl = "/";
    root = createRoot(container);
    ...
    expect(updatePayload).toEqual(
      expect.objectContaining({
        seoDescription: "x".repeat(277),
        canonicalUrl: "/",
      })
    );
  });

  it("shows the first validation error when save is blocked", async () => {
    editorLockState.isReadOnly = false;
    mockPage.canonicalUrl = "not-a-url";
    root = createRoot(container);
    ...
    expect(toastMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        description: "Must be a valid URL or root-relative path",
        variant: "destructive",
      })
    );
