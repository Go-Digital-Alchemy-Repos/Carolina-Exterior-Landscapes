...
const IMAGE_POSITION_FIELD_GROUPS = [
  {
    imageKey: "imageUrl",
    positionXKey: "imagePositionX",
    positionYKey: "imagePositionY",
  },
  {
...
const INSPECTOR_GROUP_ORDER: InspectorGroup[] = ["content", "media", "settings", "layout"];
...
function containsPlainTextMarkup(value: string) {
  return /<[^>]+>|&(nbsp|amp|quot|#39|lt|gt);/i.test(value);
}
...
function getImagePositionFieldGroup(imageKey: string) {
  return IMAGE_POSITION_FIELD_GROUPS.find((group) => group.imageKey === imageKey);
}

function normalizePositionValue(value: unknown) {
    ...
    <div className="space-y-3">
      {value.map((item, idx) => (
        <div key={idx} className="border rounded-lg p-3 space-y-2 bg-muted/20 relative">
          ...
          {schema.map((field) => {
            if (!shouldRenderConditionalField(field, item)) return null;
            if (POSITION_PICKER_KEYS.has(field.key)) return null;

            const imagePositionFieldGroup = getImagePositionFieldGroup(field.key);
            const imageUrl = imagePositionFieldGroup ? String(item[imagePositionFieldGroup.imageKey] ?? "") : "";
            const hasPositionProps = imagePositionFieldGroup
              ? schema.some((schemaField) => schemaField.key === imagePositionFieldGroup.positionXKey)
              : false;
              ...
              {field.type === "boolean" ? (
                ...
                <>
                  <CmsImageUpload
                    ...
                    data-testid={`array-item-${idx}-${field.key}`}
                  />
                  {imagePositionFieldGroup && hasPositionProps && imageUrl ? (
                    <div className="pt-2">
                      <ImagePositionPicker
                        imageUrl={imageUrl}
                        positionX={normalizePositionValue(item[imagePositionFieldGroup.positionXKey])}
                        positionY={normalizePositionValue(item[imagePositionFieldGroup.positionYKey])}
                        onPositionChange={(x, y) => {
                          const next = value.map((nextItem, nextIndex) =>
                            nextIndex === idx
                              ? {
                                  ...nextItem,
                ...
                shouldUseRichTextEditor(field) ? (
                  ...
                  <Textarea
                    value={stripHtmlForPlainText(String(item[field.key] ?? ""))}
...
function PropField({
  ...
  const plainTextValue = shouldUsePlainTextValue && containsPlainTextMarkup(strVal) ? stripHtmlForPlainText(strVal) : strVal;
  const numVal = Number(value ?? 0);
  const boolVal = propDef.key === "isActive" ? value !== false : Boolean(value);
  const actionValue = normalizeButtonActionValue(propDef.key, values);

  useEffect(() => {
    if (shouldUsePlainTextValue && plainTextValue !== strVal) {
      onChange(plainTextValue);
    }
  }, [onChange, plainTextValue, shouldUsePlainTextValue, strVal]);
...
export function BlockEditor({
  ...
  const editorProps = useMemo(() => ({ ...blockDef.defaultProps, ...props }), [blockDef.defaultProps, props]);
  ...
  const availableGroups = INSPECTOR_GROUP_ORDER.filter(
  ...
  const contextualGroups = useMemo(() => {
    ...
    return INSPECTOR_GROUP_ORDER.filter((group) => groups[group].length > 0).map((group) => ({
      group,
  ...
  const renderPropList = (propDefs: PropDef[]) =>
    propDefs.map((propDef, idx) => {
      if (!shouldRenderConditionalField(propDef, editorProps)) return null;
      if (POSITION_PICKER_KEYS.has(propDef.key)) return null;

      const imagePositionFieldGroup = getImagePositionFieldGroup(propDef.key);
      const backgroundImageUrl = imagePositionFieldGroup
        ? String(editorProps[imagePositionFieldGroup.imageKey] ?? "")
        : "";
      const hasPositionProps = imagePositionFieldGroup
      ...
      return (
        <div key={propDef.key}>
          ...
          <div className="space-y-1.5">
            <Label className="text-xs font-medium">{getDynamicPropLabel(propDef, editorProps)}</Label>
            <PropField
              propDef={propDef}
              value={editorProps[propDef.key]}
              values={editorProps}
              onChange={(val) => setProp(propDef.key, val)}
            />
            {propDef.helpText ? <p className="text-xs text-muted-foreground">{propDef.helpText}</p> : null}
          </div>
          {imagePositionFieldGroup && hasPositionProps && backgroundImageUrl && (
            <div className="mt-3">
              <ImagePositionPicker
                imageUrl={backgroundImageUrl}
                positionX={normalizePositionValue(editorProps[imagePositionFieldGroup.positionXKey])}
                positionY={normalizePositionValue(editorProps[imagePositionFieldGroup.positionYKey])}
                onPositionChange={(x, y) => {
                  onChange({
                    ...props,
                    [imagePositionFieldGroup.positionXKey]: x,
