...
export default function SystemEmailsPage() {
  ...
  const previewTemplate = useMutation({
    mutationFn: async () => {
      if (!selectedTemplate) throw new Error("Select an email template first.");
      const res = await apiRequest("POST", `/api/admin/email-templates/${selectedTemplate.slug}/preview`, {
        subject,
        htmlBody,
      });
  ...
  const sendTest = useMutation({
    mutationFn: async () => {
      if (!selectedTemplate) throw new Error("Select an email template first.");
      const res = await apiRequest("POST", `/api/admin/email-templates/${selectedTemplate.slug}/test`);
    ...
    <AdminSidebar>
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h1 className="text-2xl font-heading font-semibold" data-testid="text-system-emails-title">
              System Emails
            </h1>
          ...
          >
            <RefreshCw className={cn("mr-2 h-4 w-4", restoreTemplates.isPending && "animate-spin")} />
            Restore Defaults
          </Button>
        </div>
              ...
              <>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Use these placeholders in the subject or body.</CardDescription>
                  </CardHeader>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Render the current editor values with sample data before saving or sending a test.
                    </CardDescription>
                  </CardHeader>
                    ...
                    {previewTemplate.data ? (
                      <>
                        <Alert>
                          ...
                          <AlertDescription>Rendered subject with sample variables.</AlertDescription>
                        </Alert>
