...
// Renders text with the first mention of each city (per page render) linked to
// its location landing page. Cities already linked (tracked via context `seen`)
// and the page's own city (`excludeSlug`) are left as plain text.
export function LinkedText({ text }: { text: string }): ReactNode {
  ...
  text.replace(CITY_RE, (match: string, _city: string, offset: number) => {
    ...
    if (slug && slug !== excludeSlug && !seen.has(slug)) {
      ...
      nodes.push(
        <Link key={key++} href={`/service-areas/${slug}`} className={LINK_CLASS}>
          {match}
        </Link>,
      );
